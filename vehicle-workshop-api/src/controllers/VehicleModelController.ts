...
export const getVehicleModels = async (c: Context) => {
    try {
        const vehicleModels = await prisma.vehicleModel.findMany({ orderBy: { id: 'desc' } });
        return c.json({
            success: true,
        ...
        return c.json({ success: false, message: 'Error retrieving vehicle models' }, 500);
    }
}
...
export const createVehicleModel = async (c: Context) => {
    try {
        ...
        const brandId = typeof body['brandId'] === 'number' ? body['brandId'] : 0;
        ...
        return c.json({ success: false, message: 'Error creating vehicle model' }, 500);
    }
}
...
export const getVehicleModelById = async (c: Context) => {
    try {
        ...
        if (!vehicleModel) {
            return c.json({ success: false, message: 'Vehicle Model Not Found!' }, 404);
        }
        ...
        return c.json({ success: false, message: 'Error finding vehicle model' }, 500);
    }
}
...
export const updateVehicleModel = async (c: Context) => {
    try {
        ...
        const name = typeof body['name'] === 'string' ? body['name'] : '';
        const brandId = typeof body['brandId'] === 'number' ? body['brandId'] : undefined;
        ...
        return c.json({ success: false, message: 'Error updating vehicle model' }, 500);
    }
}
...
export const deleteVehicleModel = async (c: Context) => {
    try {
        ...
        return c.json({ success: false, message: 'Error deleting vehicle model' }, 500);
    }
}
...
import prisma from '../../prisma/client'; // Adjust the path according to your project structure
...
export const getVehicleTypes = async (c: Context) => {
    try {
        const vehicleTypes = await prisma.vehicleType.findMany({ orderBy: { id: 'desc' } });
        return c.json({
            success: true,
        ...
        return c.json({ success: false, message: 'Error retrieving vehicle types' }, 500);
    }
}
...
export const createVehicleType = async (c: Context) => {
    try {
        ...
        const modelId = typeof body['modelId'] === 'number' ? body['modelId'] : 0; // Default to 0 or another appropriate value
        ...
        return c.json({ success: false, message: 'Error creating vehicle type' }, 500);
    }
}
...
export const getVehicleTypeById = async (c: Context) => {
    try {
        ...
        if (!vehicleType) {
            return c.json({ success: false, message: 'Vehicle Type Not Found!' }, 404);
        }
        ...
        return c.json({ success: false, message: 'Error finding vehicle type' }, 500);
    }
}
...
export const updateVehicleType = async (c: Context) => {
    try {
        ...
        const name = typeof body['name'] === 'string' ? body['name'] : '';
        const modelId = typeof body['modelId'] === 'number' ? body['modelId'] : undefined;
        ...
        return c.json({ success: false, message: 'Error updating vehicle type' }, 500);
    }
}
...
export const deleteVehicleType = async (c: Context) => {
    try {
        ...
        return c.json({ success: false, message: 'Error deleting vehicle type' }, 500);
    }
}